...
def load_models():
    whisper_model = WhisperForConditionalGeneration.from_pretrained("fine_tuned_whisper_model.pth")
    whisper_processor = WhisperProcessor.from_pretrained("fine_tuned_whisper_model.pth")
    ...
    stable_diffusion_model.scheduler.num_inference_steps = 25  # Fewer steps for faster generation
...
if st.button("Record"):
    ...
    if contains_negative_sentiment(transcription):
        ...
        with st.spinner("Generating image..."):
            # Generate full resolution image (default 512x512)
